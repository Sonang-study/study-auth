...
export class TasksController {
  ...
  @Get('')
  async getTaskList(@Query('userId', ParseIntPipe) userId: number | null, @User() user) {
  ...
  @Get(':id')
  async getUser(@Param('id', ParseIntPipe) id:number, @User() user) {
    return await this.tasksService.getOne(id);
  }

  @ApiOperation({summary:'할일 종합 목표 수정 및 인증사진 추가'})
  @Put(':id')
  async updateTask(@Param('id', ParseIntPipe) id:number, @Body() updateTaskDto: UpdateTaskDto, @User() user) {
    return await this.tasksService.updateTask(id, updateTaskDto);
  }

  @ApiOperation({summary: '할일 목표 리스트'})
  @Get(':id/task-day')
  async getTaskDayList(@Param('id', ParseIntPipe) taskId:number, @User() user) {
    return await this.tasksService.getTaskDayList(taskId);
  }

  @ApiOperation({summary: '할일 목표 추가'})
  @Post(':id/task-day')
  async createTaskDay(@Param('id', ParseIntPipe) taskId:number, @Body() createTaskDayDto: CreateTaskDayDto, @User() user) {
  ...
  async getTaskDay(@Param('id', ParseIntPipe) taskId:number, @Param('taskDayId', ParseIntPipe) taskDayId:number, @User() user) {
